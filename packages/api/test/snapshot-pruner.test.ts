...
import { resetFakeDb, seedStatus } from "./support/fake-db.js";
...
describe("pruneSnapshots", () => {
  ...
  it("always keeps at least minKeep rows even if all are old", async () => {
    // All 5 rows are old
    for (let i = 0; i < 5; i++) {
      seedStatus({ updatedAt: new Date(Date.now() - hours(200 + i)) });
    }

    const deleted = await pruneSnapshots({ retentionHours: 24, minKeep: 3, intervalSeconds: 60 });
    // Must keep the 3 most recent, so only 2 can be deleted
