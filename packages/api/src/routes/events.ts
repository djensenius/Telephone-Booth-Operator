...
function callEndedData(event: BoothEvent): Partial<StoredCallSession> | null {
  ...
  const outcome = typeof outcomeRaw === "string" && CallOutcomeSchema.safeParse(outcomeRaw).success
    ? outcomeRaw
    : null;
  ...
  const recordingId = event.recordingId ?? (typeof payload.recording_id === "string" ? payload.recording_id : null);
  return {
    id: event.sessionId,
    boothId: event.boothId,
    bootId: event.bootId,
...
eventsRouter.post(
  "/",
  requireApiToken(),
  zValidator("json", BoothEventBatchSchema),
  async (c) => {
    ...
    for (const init of sessionInits.values()) {
      ...
      await db.callSession.upsert({
        ...
        update: {
          ...
          ...(init.digitsDialed !== undefined && init.digitsDialed !== null ? { digitsDialed: init.digitsDialed } : {}),
          ...(init.outcome ? { outcome: init.outcome } : {}),
          ...(init.recordingId ? { recordingId: init.recordingId } : {}),
          ...(init.durationMs !== undefined && init.durationMs !== null ? { durationMs: init.durationMs } : {}),
        },
      });
    }
