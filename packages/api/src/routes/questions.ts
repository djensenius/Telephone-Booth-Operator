...
import { QuestionCreateSchema } from "@telephone-booth-operator/shared";
...
questionsRouter.post("/", zValidator("json", QuestionCreateSchema), async (c) => {
  ...
  try {
    const question = await db.question.create({
      data: { prompt: body.prompt, audioId: body.audioFileId },
      include: { audio: true },
    });
...
questionsRouter.delete("/:id", zValidator("param", idParamSchema), async (c) => {
  ...
  if (!question || question.retiredAt) return c.json({ error: "not_found" }, 404);

  await db.question.update({ where: { id }, data: { retiredAt: new Date() } });
  return c.body(null, 204);
});

questionsRouter.get("/random", requireApiToken(), async (c) => {
  const count = await db.question.count({ where: { retiredAt: null } });
