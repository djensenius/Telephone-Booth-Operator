...
export function PhoneClientConnection({
  userSub = "anonymous",
  ...
  async function pinLanCert(): Promise<void> {
    ...
    try {
      ...
      writeDebugConnectionPrefs(next, userSub);
      setStatus(
        "LAN fingerprint pinned. Compare it with the browser certificate details when accepting the one-time LAN exception.",
      );
    } catch (error) {
      setStatus(
        error instanceof Error
          ? `Could not pin fingerprint: ${error.message}`
          : "Could not pin fingerprint.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section className="settings-connection" aria-labelledby="phone-client-connection-heading">
      ...
      <p>
        Use Tailscale first. LAN fallback uses a self-signed certificate; pin its SHA-256
        fingerprint here, then verify it in your browser certificate details before accepting the
        one-time exception.
      </p>
      <div className="settings-connection__fields">
        <label>
          ...
          <input
            type="url"
            value={prefs.tailscaleUrl}
            onChange={(event) => updateField("tailscaleUrl", event.currentTarget.value)}
            placeholder="https://phone-booth.tail-scale.ts.net"
          />
        </label>
        <label>
          LAN URL
          <input
            type="url"
            value={prefs.lanUrl}
            onChange={(event) => updateField("lanUrl", event.currentTarget.value)}
        ...
        <label>
          ...
          <input
            value={prefs.pinnedFingerprint}
            onChange={(event) => updateField("pinnedFingerprint", event.currentTarget.value)}
      ...
      <div className="debug-button-row">
        ...
        <button
          type="button"
          onClick={() => void pinLanCert()}
          disabled={busy || prefs.tailscaleUrl.length === 0}
        >
          Pin LAN cert
        </button>
