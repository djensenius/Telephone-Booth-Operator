  ...
  const points =
    buckets.length === 0
      ...
      : buckets
          .map(
            (bucket, index) =>
              `${(index / Math.max(1, buckets.length - 1)) * 80},${24 - (bucket.count / max) * 22}`,
          )
          .join(" ");
  return (
    <svg
      ...
      viewBox="0 0 80 26"
      role="img"
      aria-label={usage.isLoading ? "Loading token usage" : `${buckets.length} usage buckets`}
    >
...
function TokenRow({
  ...
  return (
    <tr>
      ...
      <td>
        <UsageSparkline tokenId={token.id} />
      </td>
      <td>
        <button type="button" disabled={token.revokedAt !== null} onClick={() => onRevoke(token)}>
          {token.revokedAt === null ? "Revoke" : "Revoked"}
        </button>
      </td>
    </tr>
  );
      ...
      {rows.length === 0 ? null : (
        <div className="feature-table-wrap">
          <table className="feature-table">
            ...
            <tbody>
              {rows.map((token) => (
                <TokenRow key={token.id} token={token} onRevoke={setRevokeToken} />
              ))}
            </tbody>
          </table>
        </div>
        ...
        >
          ...
          <div className="debug-button-row">
            <button
              type="button"
              onClick={() =>
                void revoke.mutateAsync(revokeToken.id).then(() => setRevokeToken(null))
              }
            >
              Confirm revoke
            </button>
            <button type="button" onClick={() => setRevokeToken(null)}>
              Cancel
            </button>
          </div>
        </section>
