...
const operator = { id: "user-1", email: "operator@example.com", name: "Jane Operator", groups: ["operators"], providerName: "Authentik" };
...
const question = { id: questionId, prompt: "What did the city sound like today?", createdAt: "2026-01-01T00:00:00.000Z", audio: { url: "https://media.example/question.flac", sha256: sha, durationMs: 12000 } };
const questionTwo = { ...question, id: questionTwoId, prompt: "Who are you calling?" };
const message = { id: messageId, status: "received", questionId, notes: "front step", createdAt: "2026-01-02T00:00:00.000Z", receivedAt: "2026-01-02T00:01:00.000Z", audio: { url: "https://media.example/message.flac", sha256: sha, durationMs: 9000 } };
const token = { id: tokenId, name: "booth client", last4: "1234", createdAt: "2026-01-03T00:00:00.000Z", expiresAt: null, lastUsedAt: "2026-01-04T00:00:00.000Z", revokedAt: null };
...
const server = setupServer(
  ...
  http.get("http://localhost/v1/status", () => HttpResponse.json({ state: "idle", updatedAt: "2026-01-05T00:00:00.000Z", currentQuestionId: null, currentMessageId: null, lastError: null })),
  http.get("http://localhost/v1/status/history", () => HttpResponse.json({ items: [{ state: "idle", updatedAt: "2026-01-05T00:00:00.000Z", currentQuestionId: null, currentMessageId: null, lastError: null }] })),
  http.get("http://localhost/v1/questions", () => HttpResponse.json({ items: createdQuestion ? [questionTwo, question] : [question], nextCursor: null })),
  http.post("http://localhost/v1/uploads/sas", () => HttpResponse.json({ uploadUrl: "https://blob.example/upload", blobName: "questions/aa/file.flac", expiresAt: "2026-01-01T00:10:00.000Z", audioFileId }, { status: 201 })),
  ...
  http.get("http://localhost/v1/api-tokens", () => HttpResponse.json([{ ...token, revokedAt: revokedToken ? "2026-01-05T00:00:00.000Z" : null }])),
  http.post("http://localhost/v1/api-tokens", () => HttpResponse.json({ ...token, plaintext: "booth-token-plaintext", lastUsedAt: undefined, revokedAt: undefined }, { status: 201 })),
  http.delete("http://localhost/v1/api-tokens/:id", () => {
    revokedToken = true;
    return new HttpResponse(null, { status: 204 });
  }),
  http.get("http://localhost/v1/api-tokens/:id/usage", () => HttpResponse.json([{ date: "2026-01-04", count: 1 }])),
...
function installBrowserStubs(): void {
  ...
  Object.defineProperty(window.HTMLCanvasElement.prototype, "getContext", { configurable: true, value: vi.fn(() => null) });
...
describe("Status feature", () => {
  ...
  it("shows an empty state when no snapshots exist", async () => {
    server.use(http.get("http://localhost/v1/status/history", () => HttpResponse.json({ items: [] })));
    renderPath("/status");
    expect(await screen.findByText("On hook")).toBeTruthy();
  });

  it("shows a busy placard on status errors", async () => {
    server.use(http.get("http://localhost/v1/status", () => HttpResponse.json({ error: "busy" }, { status: 500 })));
    renderPath("/status");
...
describe("Questions feature", () => {
  ...
  it("uploads audio and creates a question", async () => {
    renderPath("/questions");
    fireEvent.click(await screen.findByText("New question"));
    fireEvent.change(screen.getByLabelText("Prompt"), { target: { value: "Who lifted the receiver?" } });
    fireEvent.change(screen.getByLabelText("Audio file (FLAC)"), { target: { files: [new File(["audio"], "q.flac", { type: "audio/flac" })] } });
  ...
  it("shows the empty library copy", async () => {
    server.use(http.get("http://localhost/v1/questions", () => HttpResponse.json({ items: [], nextCursor: null })));
    renderPath("/questions");
...
describe("About feature", () => {
  ...
  it("links to GitHub", async () => {
    renderPath("/about");
    expect((await screen.findByText("GitHub")).closest("a")?.getAttribute("href")).toContain("github.com");
  });
...
describe("API client helpers", () => {
  ...
  it("hashes blobs as lowercase sha256", async () => {
    await expect(sha256Hex(new Blob(["a"]))).resolves.toBe("ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb");
  });
});
