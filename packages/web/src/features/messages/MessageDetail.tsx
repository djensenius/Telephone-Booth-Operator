...
function moderationVariant(moderation: Moderation | null | undefined): { label: string; variant: string } {
  ...
  if (moderation.recommendation === "reject") return { label: "Flagged for rejection", variant: "reject" };
...
function TranscriptCard({ message, onRetranscribe, retranscribing, retranscribeError }: TranscriptCardProps): JSX.Element {
  ...
  return (
    <section className="feature-card feature-card--wide">
      ...
      {transcription === null ? (
        ...
        <p className="feature-error">Transcription failed{transcription.error ? `: ${transcription.error}` : ""}.</p>
      ) : (
        <>
          <p className="feature-transcript-body">{transcription.text ?? ""}</p>
          <dl className="debug-kv-grid debug-kv-grid--compact">
            <div><dt>Provider</dt><dd>{transcription.provider}{transcription.model ? ` · ${transcription.model}` : ""}</dd></div>
            <div><dt>Language</dt><dd>{transcription.language ?? "—"}</dd></div>
            <div><dt>Completed</dt><dd>{formatDateTime(transcription.completedAt)}</dd></div>
            {transcription.latencyMs === null || transcription.latencyMs === undefined ? null : (
              <div><dt>Latency</dt><dd>{`${Math.round(transcription.latencyMs)} ms`}</dd></div>
            )}
          </dl>
        </>
      )}
...
function categoryRows(categories: Moderation["categories"]): readonly { name: string; score: number }[] {
...
function ModerationCard({ message, onRemoderate, remoderating, remoderateError }: ModerationCardProps): JSX.Element {
  const moderation = message.latestModeration ?? null;
  const transcription = message.latestTranscription ?? null;
  const canRun = transcription?.status === "succeeded" && (transcription.text?.trim().length ?? 0) > 0;
  ...
  return (
    <section className="feature-card feature-card--wide">
      ...
      {moderation === null ? (
        <p className="feature-empty">{canRun ? "No moderation yet." : "Waiting on transcription."}</p>
      ) : moderation.status === "pending" ? (
        <p className="feature-empty">Moderation in progress…</p>
      ) : moderation.status === "failed" ? (
        <p className="feature-error">Moderation failed{moderation.error ? `: ${moderation.error}` : ""}.</p>
      ) : (
        <>
          {moderation.reasonSummary ? <p className="feature-transcript-body">{moderation.reasonSummary}</p> : null}
          <dl className="debug-kv-grid debug-kv-grid--compact">
            <div><dt>Provider</dt><dd>{moderation.provider}{moderation.model ? ` · ${moderation.model}` : ""}</dd></div>
            <div><dt>Recommendation</dt><dd>{moderation.recommendation ?? "—"}</dd></div>
            <div><dt>Max score</dt><dd>{moderation.maxScore === null || moderation.maxScore === undefined ? "—" : moderation.maxScore.toFixed(3)}</dd></div>
            <div><dt>Flagged</dt><dd>{moderation.flagged === null || moderation.flagged === undefined ? "—" : moderation.flagged ? "Yes" : "No"}</dd></div>
            <div><dt>Completed</dt><dd>{formatDateTime(moderation.completedAt)}</dd></div>
          </dl>
          {rows.length === 0 ? null : (
            <table className="feature-table">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.name}><td>{row.name}</td><td>{row.score.toFixed(3)}</td></tr>
                ))}
              </tbody>
            </table>
          )}
...
function HistoryCard({ transcriptions }: HistoryCardProps): JSX.Element | null {
  ...
  return (
    <section className="feature-card feature-card--wide">
      <details>
        <summary><h2>History ({prior.length} prior attempt{prior.length === 1 ? "" : "s"})</h2></summary>
        <table className="feature-table">
          <caption>Transcription attempts</caption>
          <thead><tr><th>Created</th><th>Provider</th><th>Status</th><th>Snippet</th></tr></thead>
          <tbody>
            {prior.map((entry) => (
              <tr key={entry.id}>
                <td>{formatDateTime(entry.createdAt)}</td>
                <td>{entry.provider}{entry.model ? ` · ${entry.model}` : ""}</td>
                <td>{entry.status}</td>
                <td>{entry.text === null || entry.text === undefined || entry.text.length === 0 ? "—" : entry.text.length > 80 ? `${entry.text.slice(0, 79)}…` : entry.text}</td>
              </tr>
            ))}
          </tbody>
        </table>
