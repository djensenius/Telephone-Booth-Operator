...
flowchart LR
  ...
  BusyWorker[External BUSY Bar companion] -->|Operator REST + WS| API
  BusyWorker -->|BUSY Cloud HTTP + WS| BusyBar[BUSY Bar\nstatus monitor]
...
6. The separately deployed
   [BUSY Bar companion](busy-bar-monitor.md) can consume authenticated
   status/system REST + WebSocket data plus aggregate-only daily counters. It
   renders the physical display through BUSY Cloud and never calls booth
   mutation operations. Keeping it outside this repository and API process
   prevents horizontally scaled replicas from racing.
