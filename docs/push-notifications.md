...
2. When a message hits `POST /v1/messages/{id}/complete`, the API computes the
   ...
   `pending`) and schedules a best-effort alert for that arrival. The alert
   body reports the current count and uses the stable
   `message-moderation-queue` collapse identifier, so each arrival alerts the
   operator while replacing the previous queue notification instead of adding
   another Notification Center entry. Overlapping arrivals are serialized and
   may coalesce to the newest valid count. The alert intentionally omits an
   `aps.badge` value; durable badge-only pushes remain the authoritative count.
   The app removes the delivered queue alert when the operator opens matching
   content.
